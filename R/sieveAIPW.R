...
covEstAIPW <- function(time, find, mark, txInd, auxMiss, aux, phiHat, lambdaHat, gammaHat){
  n <- length(time)
  ...
  pi.all <- glm(r ~ txInd.f*auxMiss.f, family=binomial)$fitted
  if (!is.null(na.idx)){
    pi <- pi.all[-na.idx]
  } else {
    pi <- pi.all
...
# 'txInd' is the treatment group indicator (1 if treatment, 0 if control)
# 'aux'
densRatioAIPW <- function(mark, txInd, aux){
  V <- cbind(1,mark)
  ...
  #	param.old <- densRatioIPW(mark, txInd, aux)$coef
...
#' 
#' \code{sieveAIPW} conducts estimation and testing of the multivariate mark-specific hazard ratio 
...
#' @details 
#' 
#' @return \code{sieveAIPW} returns an object of class "sieveAIPW" which can be processed by 
#' \code{\link{summary.sieveAIPW}} to obtain or print a summary of the results. An object of class
#' "sieveAIPW" is a list containing the following components:
