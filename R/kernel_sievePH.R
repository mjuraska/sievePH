...
#' \item \code{cBproc1}: a data frame with containing mark values under column {\code Mark}, standardized mark values under column {\code Standardized mark}, 
#' test processes \eqn{Q^{(1)}(v)} for \code{H10} based on observed data sequence under column (\code{Observed}) and independent sets of normal samples under columns (\code{S1, S2, \cdots, S_{nboot}}).
#' \item \code{cBproc2}: a data frame with containing mark values under column {\code Mark}, standardized mark values under column {\code Standardized mark}, 
#' test processes \eqn{Q^{(2)}(v)} for \code{H20} based on observed data sequence under column (\code{Observed}) and independent sets of normal samples under columns (\code{S1, S2, \cdots, S_{nboot}}).
#' }
#' 
#' @references 
